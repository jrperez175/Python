import argparse #Crea argumentos personalizados al momento de ejecutar un programa
import logging
...
def main(filename):
    ...
    #df = _remove_new_lines_from_body(df) #Definido Profesor
    df = _remove_new_lines_from_body_2(df) #Definido Alumnos
    df = _tokenize_columns(df,['body','title'])
...
def _remove_new_lines_from_body(df):
    ...
    stripped_body = (df
                    ...
                    .apply(lambda list_letters: ''.join(list_letters))
    )
    df['body'] = stripped_body
    return df

def _remove_new_lines_from_body_2(df):
    logger.info('Remove new lines from body')

    stripped_body = (df['body']
                    .dropna()
                    .apply(str)
...
def _tokenize_columns(df,columns_name):
    columns = []
    ...
    logger.info(f'Tokenizing columns {[column for column in columns]}')

    for column in columns:
        df['n_tokens_' + column] = (df
                                .dropna()
                                .apply(lambda row: nltk.word_tokenize(row[column]), axis = 1)
                                .apply(lambda tokens: list(filter(lambda token: token.isalpha(),tokens)))
                                .apply(lambda list_tokens: map(lambda token: token.lower(),list_tokens))
